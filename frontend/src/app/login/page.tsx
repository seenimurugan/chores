...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    api.me().then((u) => router.replace(u.role === 'ADMIN' ? '/admin' : '/')).catch(() => {});
  }, [router]);
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      setToken(r.token);
      router.replace(r.user.role === 'ADMIN' ? '/admin' : '/');
    } catch (err: any) {
    ...
    <div className="min-h-screen flex items-center justify-center px-4">
      ...
      >
        <div className="text-center">
          <div className="mx-auto w-12 h-12 rounded-2xl bg-brand-500 text-white flex items-center justify-center text-2xl">✓</div>
          <h1 className="mt-3 text-xl font-semibold">Chores</h1>
